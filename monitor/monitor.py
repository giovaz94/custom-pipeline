...
class Logger:
    ...
    def log(self) -> None:
        ...
        while True:
            ...
            completed = self._execute_prometheus_query("sum(rate(message_analyzer_complete_message[10s]))")
            latency = self._execute_prometheus_query("sum(rate(http_response_time_sum[10s]))")
