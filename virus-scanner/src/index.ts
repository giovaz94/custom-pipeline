import {addInQueue, closeConnection, dequeue, startConsumer, TaskType, pendingPromises, queue} from "./queue/queue";
...
startConsumer(queueName, async (channel) => {
   while(true){
      ...
      const targetType = 'messageanalyzer.req'; //isVirus ? 'messageanalyzer.req' : 'attachmentman.req';
